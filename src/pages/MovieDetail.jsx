...
const MovieDetail = () => {
    ...
    useEffect(() => {
        const movies = i18n.language === "es" ? moviesES : moviesEN;
    ...
    if (!movie) {
        return <p className="text-center text-red-500">{t('movie_not_found')}</p>;
    }
        ...
        <div className="w-full flex flex-col items-center justify-center my-8 py-10 px-4">
            <div className="max-w-[1300px] w-full px-4 grid grid-cols-1 md:grid-cols-8 gap-8 items-start">
                ...
                <div className="col-span-2">
                    <div className="w-full h-105 overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-500">
                        <img
                            src={`/${movie.cover}`}
                            alt={movie.name}
                ...
                <div className="col-span-3 space-y-4">
                    <h1 className="text-4xl text-[#004D43] font-semibold">{movie.name}</h1>
                    <hr className="w-full h-1 bg-[#004D43] mt-2 mb-1" />

                    <p className="text-lg text-gray-700">{movie.description}</p>

                    <p className="text-lg text-gray-700 mt-4 font-semibold underline">{movie.author}</p>
                    <p className="text-lg text-gray-700">{movie.volume}</p>

                    <div className="flex flex-wrap gap-2 mt-4">
                        {movie.type.map((category, index) => (
                ...
                <div className="col-span-3 flex flex-col items-start justify-center space-y-4">
                    <p className="text-4xl text-left font-bold text-[#004D43]">{movie.price}€</p>
            ...
            <div className="max-w-[1300px] w-full">
                <h2 className="text-4xl text-[#004D43] font-semibold text-center mt-10">{t('watch_movie')}</h2>

                <div className="w-full mt-8 flex justify-center items-center">
                    <div className="relative w-full max-w-[800px] h-[500px] group">
                        <video controls className="w-full h-full object-cover rounded-lg shadow-2xl transition-all duration-300 group-hover:scale-105 group-hover:shadow-2xl">
            ...
            {isShareOpen && (
                <div className="fixed inset-0 bg-black/70 bg-opacity-50 flex justify-center items-center z-10">
                    <div className="bg-white p-6 rounded-lg shadow-lg w-110">
                        <h2 className="text-xl font-semibold text-center mb-4">{t('share_movie')}</h2>
                        ...
                        <div className="flex justify-between">
                            ...
                            <SocialIcon className="transition-all duration-300 transform hover:scale-110 hover:cursor-pointer" onClick={() => window.open(`mailto:?subject=Check out this movie&body=${window.location.href}`, '_blank')} network="email" fgColor="#fff" style={{ height: 40, width: 40 }} />
